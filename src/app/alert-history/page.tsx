...
export default function AlertHistoryPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-500">
            <span>Home</span>
            <span>›</span>
            <span className="text-cyan-600 dark:text-cyan-400">Alert History</span>
          </div>
          <div className="flex items-center gap-2 text-xs">
            {refreshing ? (
              ...
              <>
                ...
                <span className="text-slate-500 dark:text-slate-500">Auto-updating every 15s</span>
              </>
            )}
          </div>
        </div>
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Alert History</h1>
        <p className="text-slate-600 dark:text-slate-400">Review all alerts sent for your monitored APIs</p>
      </div>
      ...
      {/* Filters */}
      <div className="flex gap-4 items-center p-4 bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm dark:shadow-none">
        <Filter className="h-5 w-5 text-slate-400 dark:text-slate-400" />
      ...
      <div className="space-y-4">
        {alerts.length === 0 ? (
          <div className="text-center py-12 bg-slate-100 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800 rounded-xl">
            ...
            <h3 className="text-lg font-semibold text-slate-600 dark:text-slate-400 mb-2">No Alerts Found</h3>
            <p className="text-slate-500 dark:text-slate-500">No alerts match your current filters</p>
          </div>
        ) : (
          alerts.map((alert) => (
    ...
    <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-6 hover:border-purple-500/30 transition-all shadow-sm dark:shadow-none">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400">{getReasonLabel(alert.alert_reason)}</p>
          {alert.endpoint_path && (
            <p className="text-xs text-cyan-700 dark:text-cyan-400 font-mono mt-1">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-slate-900 dark:text-white capitalize">{alert.status}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Metadata Row */}
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 py-4 border-t border-slate-200 dark:border-slate-800">
        <div>
        ...
        {alert.error_message && (
          <div className="col-span-2 lg:col-span-1">
            ...
            <p className="text-sm text-red-400 dark:text-red-400 font-mono">
              {alert.error_message}
            </p>
          </div>
        )}
      ...
      {alert.payload && (
        <div className="pt-4 border-t border-slate-200 dark:border-slate-800">
          <button
            ...
            className="text-sm text-purple-400 dark:text-purple-400 hover:text-purple-300 dark:hover:text-purple-300 font-semibold flex items-center gap-2"
          >
            <ExternalLink className="h-4 w-4" />
            {expanded ? 'Hide Details' : 'View Details'}
          </button>
          {expanded && (
            <pre className="mt-3 p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-xs text-slate-700 dark:text-slate-300 overflow-x-auto">
