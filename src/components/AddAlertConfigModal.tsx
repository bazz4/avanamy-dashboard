...
export function AddAlertConfigModal({ isOpen, onClose, onSuccess }: AddAlertConfigModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 dark:bg-black/70 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl shadow-purple-500/20 max-w-2xl w-full max-h-[90vh] overflow-hidden" role="dialog" aria-modal="true" aria-labelledby="add-alert-modal-title">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-800">
          <div>
            <h2 id="add-alert-modal-title" className="text-2xl font-bold text-slate-900 dark:text-white">Add Alert Configuration</h2>
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">Configure alerts for API monitoring</p>
          </div>
          <button
            ref={closeButtonRef}
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-180px)]">
          ...
          <div>
            <label htmlFor="watched-api-select" className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
              Watched API *
            </label>
            <select
              id="watched-api-select"
              ...
              disabled={loading}
              aria-required="true"
              aria-invalid={!!errors.watched_api_id}
              aria-describedby={errors.watched_api_id ? "watched-api-error" : undefined}
            >
            ...
            {errors.watched_api_id && (
              <p id="watched-api-error" className="text-sm text-red-400 mt-1" role="alert">{errors.watched_api_id}</p>
            )}
          </div>
              ...
              <div className="grid grid-cols-3 gap-3" role="group">
                {(['email', 'webhook', 'slack'] as const).map((type) => (
                  <button
                    ...
                    aria-pressed={formData.alert_type === type}
                  >
                    {type === 'email' && <Mail className="h-4 w-4" aria-hidden="true" />}
                    {type === 'webhook' && <Webhook className="h-4 w-4" aria-hidden="true" />}
                    {type === 'slack' && <MessageSquare className="h-4 w-4" aria-hidden="true" />}
          ...
          {/* Destination */}
          <div>
            <label htmlFor="destination-input" className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
              Destination *
            </label>
            <div className="relative">
              <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" aria-hidden="true">
                {getDestinationIcon()}
              </div>
              <input
                ...
                className="w-full pl-12 pr-4 py-3 bg-slate-100 dark:bg-slate-900/50 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                aria-required="true"
            ...
            {errors.destination && (
              <p id="destination-error" className="text-sm text-red-400 mt-1" role="alert">{errors.destination}</p>
            )}
          </div>
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-3 p-4 bg-slate-100 dark:bg-slate-800 rounded-lg cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-750 transition-colors">
                ...
                <div className="flex-1">
                  <div className="font-semibold text-slate-900 dark:text-white">Breaking Changes</div>
                  <div className="text-xs text-slate-600 dark:text-slate-400">Alert when breaking API changes are detected</div>
                </div>
              </label>
              ...
              <label className="flex items-center gap-3 p-4 bg-slate-100 dark:bg-slate-800 rounded-lg cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-750 transition-colors">
                ...
                />
                <div className="flex-1">
                  <div className="font-semibold text-slate-900 dark:text-white">Non-Breaking Changes</div>
                  <div className="text-xs text-slate-600 dark:text-slate-400">Alert on any API spec changes</div>
                </div>
              </label>
              ...
              <label className="flex items-center gap-3 p-4 bg-slate-100 dark:bg-slate-800 rounded-lg cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-750 transition-colors">
                ...
                />
                <div className="flex-1">
                  <div className="font-semibold text-slate-900 dark:text-white">Endpoint Failures</div>
                  <div className="text-xs text-slate-600 dark:text-slate-400">Alert when endpoints become unhealthy</div>
                </div>
              </label>
              ...
              <label className="flex items-center gap-3 p-4 bg-slate-100 dark:bg-slate-800 rounded-lg cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-750 transition-colors">
                ...
                />
                <div className="flex-1">
                  <div className="font-semibold text-slate-900 dark:text-white">Endpoint Recovery</div>
                  <div className="text-xs text-slate-600 dark:text-slate-400">Alert when endpoints recover from failures</div>
                </div>
              </label>
            </div>
          </div>
